...
Model::Model(const std::string& fileName, ActionMode action_space){
    ...
    for (size_t i = 0; i < temp_board.size(); ++i) {
        for (size_t j = 0; j < temp_board.size(); ++j) {
...
void Model::printState(ABS::Gamestate* state) {
    ...
    for (size_t i = 0; i < noise_map.size(); ++i) {
        for (size_t j = 0; j < noise_map.size(); ++j) {
...
std::vector<int> Model::getActions_(ABS::Gamestate* uncasted_state)  {
    ...
    for (size_t i = 0; i < noise_map.size() * noise_map.size(); ++i) {
        bool legal_action = (action_mode == ActionMode::SAVE_ONLY && getCell(state->board, i / noise_map.size(), i % noise_map.size(), map_width)) ||
            (action_mode == ActionMode::REVIVE_ONLY && !getCell(state->board, i / noise_map.size(), i % noise_map.size(), map_width)) ||
