...
std::pair<std::vector<double>,double> SkillsTeachingModel::applyAction_(ABS::Gamestate* uncasted_state, int action, std::mt19937& rng){
    ...
    for(size_t i = 0; i < prerequisites.size(); i++) {
        ...
        if(student_turn && (askProb && targeted_skill == static_cast<int>(i)) && oldState.proficiencyHigh[i]) {
            ...
            std::bernoulli_distribution bernoulli(prob_per_pre_med[i] * prerequisites[i].size());
            answered_right = bernoulli(rng);
            outcomeProb *= answered_right ? (prob_per_pre_med[i]* prerequisites[i].size()) : (1.0 - prob_per_pre_med[i] * prerequisites[i].size());
            ...
            std::bernoulli_distribution bernoulli(prob_per_pre[i] * prerequisites[i].size());
            answered_right = bernoulli(rng);
            outcomeProb *= answered_right ? (prob_per_pre[i]* prerequisites[i].size()) : (1.0 - prob_per_pre[i] * prerequisites[i].size());
        }else {
